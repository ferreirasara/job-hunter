...
import { QUICKIN_URLS } from '../urls/urls';
...
export default class QuickinScraper extends ScraperInterface {
  ...
  private async getUrls(page: Page): Promise<JobInitialData[]> {
    ...
    for (const url of QUICKIN_URLS) {
      try {
        await page.goto(url);
        await page.waitForSelector('th > a');
        await sleep(100);
        const urls: string[] = await page?.$$eval('th > a', (el) =>
          el?.map((cur) => cur?.href),
        );
        result?.push(
          ...urls?.map((url) => ({
  ...
  private async getDetails(
    ...
    for (let i = 0; i < urlsLength; i++) {
      try {
        ...
        const company = obj?.url?.split('quickin.io/')?.[1]?.split('/')?.[0];
