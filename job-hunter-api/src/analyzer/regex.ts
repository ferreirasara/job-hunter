export const SKILLS_REGEX = {
  ACCESSIBILITY: [/acessibilidade/i, /\bwcag\b/i],
  AG_GRID: [/ag( |-|\.)grid/i],
  AGILE: [
    /\bagile\b/i,
    /\bscrum\b/i,
    /\bjira\b/i,
    /confluence/i,
    /trello/i,
    /kanban/i,
    /pair programming/i,
    /(praticas|modelo|ferramenta(s)?) ag(eis|il)/i,
    /clean architectures/i,
    /(metodo(logia(s)?)?(s)?|desenvolvimento|cultura|framework(s)?|design) ag(eis|il)/i,
  ],
  AI_AGENTS: [
    /\bgemini\b/,
    /\bgpt\b/,
    /\bopenai\b/,
    /\banthropic\b/,
    /\bprompt\b/,
    /\b(ferramenta(s)?|recurso(s)?) de (ia|inteligencia artificial)\b/,
    /ai agents/i,
    /\bn8n\b/i,
    /claude( code)?/i,
    /\bcursor\b/i,
    /\bferramentas de ia\b/i,
    /ia (voltada ao(s)? desenvolvimento(s)?|generativa)/i,
    /\bcopilot\b/i,
    /\bcodex\b/i,
    /\bchatgpt\b/i,
    /agente(s)? inteligente(s)?/i,
  ],
  AJAX: [/\bajax\b/i],
  ANGULAR: [/angular(( |-|\.)?js)?/i],
  ANTD: [/\bant( |-|\.)?(d(esign)?)\b/i, /biblioteca(s)? de ui/i, /ui( |-|\.)?kit/i],
  API: [
    /((\bapi(s)?\b )|\b)rest(ful|\b)(?!.)?( api(s)?)?/i,
    /\bapi(s)?\b/i,
    /soap/i,
    /graphql/i,
    /\bexpress(( |-|\.)?js)?\b/i,
    /fast( |-|\.)?api/i,
  ],
  APOLLO: [/apollo/i],
  ARTIFICIAL_INTELLIGENCE: [
    /nlp/i,
    /watson/i,
  ],
  AUTHENTICATION: [/jwt/i, /oauth/i, /authentication/i, /autenticacao/i],
  BACHELORS_DEGREE: [
    /bachelor's degree/i,
    /computer science/i,
    /bacharelado/i,
    /ciencia(s)? da computacao/i,
    /(gradua(do|cao)|superior)(\s|\s-\s)complet(a|o)/i,
    /(?<!nao ter )(ensino|formacao) (superior| na area de tecnologia)/i,
    /possuir formacao/i,
    /grau de cs/i,
    /sistema(s)? de informacao/i,
    /analise (e desenvolvimento )?de sistema(s)?/i,
  ],
  BACKBONE: [/backbone/i, /marionette( |-|\.)?js/i],
  BLAZOR: [/blazor/i],
  BLOCKCHAIN: [/blockchain/i, /crypto/i,],
  BLUEPRINTJS: [/blue( |-|\.)?print(js)?/i],
  BOOTSTRAP: [/boo(t)?strap/i, /biblioteca(s)? de ui/i],
  C: [/\bc\b(?!-)/i],
  CERTIFICATIONS: [
    /\b(?<!reembolso por )certificações\b/i,
    /certification(s)?/i,
    /certificac(oes|ao) relevante(s)?/i,
    /certificac(oes|ao) em/i,
  ],
  CHARTS: [/recharts/i, /bizcharts/i, /charts( |-|\.)?js/i, /\bd3\b/i],
  COBOL: [/cobol/i],
  CODE_MAINTAINABILITY: [/eslint/i, /prettier/i, /lint/i],
  CODE_REVIEW: [/code( |-|\.)?review/i],
  CODE_VERSIONING: [
    /\bgit(( |-|\.)?(hub|lab)?)?\b/i,
    /bitbucket/i,
    /\bsvn\b/i,
    /(controle|padrao|fluxo) de vers(ionamento|ao)/i,
    /version control/i,
    /(versionamento|versionamiento) de codigo(s)?/i,
  ],
  COLLABORATIVE_TOOLS: [/notion/i, /slack/i, /ferramentas colaborativas/i],
  CLEAN_CODE: [/clean code/i],
  CPLUSPLUS: [/c\+\+/i, /c(\s|-)?plus(\s|-)?plus/i],
  CSHARP: [/c#/i, /csharp/i, /\blinq\b/i],
  CSS: [/\b(s)?css(3)?\b/i, /flexbox/i, /(?<!show\s)\bless\b/i],
  CYBER_SECURITY: [
    /seguranca( e monitoramento)? de rede/i,
    /pentest/i,
    /firewall(s)?/i,
    /criptografia/i,
    /c(y|i)berseguranca/i,
    /application security/i,
    /cybersecurity/i,
  ],
  DART: [/dart/i],
  DATA: [/\bjson\b/i, /\bxml\b/i, /data structures/i, /yaml/i],
  DB: [
    /(banco|base)(s)? de dados/i,
    /mongo( |-|\.)?(db)?/i,
    /(ms-|microsoft )?sql( |-|\.)?(server)?/i,
    /conteinerizacao/i,
    /postgre(s)?(ql)?/i,
    /my(\s|-)?sql/i,
    /firebase/i,
    /redis/i,
    /no( |-|\.)?sql/i,
    /\bmssql\b/i,
    /dynamodb/i,
    /\borm\b/i,
    /oracle/i,
    /hibernate/i,
    /mariadb/i,
    /sqlite/i,
    /bases relacionais e nao relacionais/i,
    /sequelize/i,
    /supabase/i,
    /datomic/i,
  ],
  DELPHI: [/delphi/i, /\bvcl\b/i],
  DESIGN: [
    /design de interfaces/i,
    /photoshop/i,
    /illustrator/i,
    /sketch/i,
    /figma/i,
    /fireworks/i,
    /ilustrator/i,
  ],
  DESIGN_SYSTEM: [/design system/i],
  DEV_OPS: [
    /docker/i,
    /\baws\b/i,
    /kubern(e|a)t(e)?s/i,
    /terraform/i,
    /azure/i,
    /jenkins/i,
    /\bci(\/|-|\s|\\|\se\s)?cd\b/i,
    /container(s)?/i,
    /devops/i,
    /git( |-|\.)?(hub)?(-| )(actions|flow)/i,
    /lambda(s)?/i,
    /\becs\b/i,
    /\bec2\b/i,
    /maven/i,
    /cloudwatch/i,
    /amazon documentdb/i,
    /\brds\b/i,
    /kibana/i,
    /graphana/i,
    /apache/i,
    /nginx/i,
    /\bgcp\b/i,
    /websphere/i,
    /\btfs\b/i,
    /fortify/i,
    /sonar/i,
    /fagarte/i,
    /google cloud/i,
    /circleci/i,
    /hospedagem em nuvem/i,
    /vercel/i,
    /appdynamics/i,
    /dynatrace/i,
    /datadog/i,
    /\belk\b/i,
    /infraestrutura (e monitoramento|de ti)/i,
    /infra( |-|\.)?estrutura/i,
    /continuous deployment/i,
    /pipelines de deploy/i,
    /ambientes em nuvem/i,
    /sentry/i,
    /\bhelm\b/i,
    /gcloud/i,
    /grafana/i,
    /openshift/i,
  ],
  DJANGO: [/django/i],
  DOT_NET: [
    /asp(.\s|\s.)?net/i,
    /\b(\.)?net\b/i,
    /dot( |-|\.)?net/i,
    /\basp\b/i,
    /\bwinforms\b/i,
  ],
  DRUPAL: [/drupal/i],
  ECOMMERCE: [
    /shop(i)?fy/i,
    /linx/i,
    /tray(corp)?/i,
    /vtex/i,
    /(oracle|sap) commerce/i,
    /magento/i,
  ],
  ELASTIC_SEARCH: [/elastic(\s)?search/i],
  ELECTRON: [/\belectron\b/i],
  ELIXIR: [/elixir/i],
  EMBER: [/\bember\b/i],
  ENGLISH: [/\bingles\b/i, /english/i, /lingua inglesa/],
  FEATURE_FLAG: [/feature flag(s)?/i, /posthog/i, /growthbook/i, /launchdarkly/i, /unleash/i],
  FLASK: [/flask/i],
  FLUTTER: [/f(l)?utter/i, /widgetbook/i],
  FRONTEND_BUILD_TOOLS: [/webpack/i, /babel/i, /\vite/i, /\bparcel\b/i, /front( |-|\.)?end build/i, /ferramentas de build/i],
  FULL_STACK: [
    /full( |-|\.)?stack/i,
    /(back|front)( |-|\.)?(end)?( (e|ao|quanto o) |, )(back|front)( |-|\.)?(end)?/i,
    /(back|front)( |-|\.)?end & (front|back)( |-|\.)?end/i,
  ],
  GAME_ENGINE: [/\bunity\b/i],
  GATSBY: [/gatsby/],
  GOOD_PRACTICES: [
    /clean(\s)?(code|architecture)/i,
    /\bdry\b/i,
    /mvc/i,
    /mvvm/i,
    /(boas|melhores) praticas( de desenvolvimento)?/i,
    /orientacao a objetos/i,
    /padroes de (desenvolvimento|design|projeto)/i,
    /domain driven design/i,
    /\bddd\b/i,
    /\buml\b/i,
    /codigo (limpo|seguro)/i,
    /design(er)? patterns/i,
    /\bsolid\b/i,
    /documentacao/i,
    /\boop\b/i,
    /\bpoo\b/i,
    /desing partners/i,
    /componentizacao/i,
    /qualidade de codigo/i,
    /(codigo|soluc(ao|oes)) escalave(l|is)/i,
    /principios solidos/i,
    /codigo clean/i,
    /code quality/i,
    /attention to detail/i,
  ],
  GOLANG: [/go( |-|\.)?lang/i, /desenvolv(imento|edor) (com )?go\b/i],
  GRAPHQL: [/graph( |-|\.)?ql/i],
  HTML: [/htm(l)?(5)?/i],
  INTEGRATIONS: [/integracoes (de|com) (servico|api)(s)? de terceiros/i],
  IONIC: [/\bionic\b/i],
  JAVA: [
    /java(?!( |-|\.)?script)/i,
    /java\d/i,
    /\bspring(( |-|\.)?boot)?\b/i,
    /eclipse/i,
    /struts/i,
    /mockmvc/i,
    /groovy/i,
    /jasper/i,
    /quarkus/i,
    /netbeans/i,
  ],
  JAVASCRIPT: [
    /java( |-|\.)?script/i,
    /^(.|-)?js\b/i,
    /es(5|6|7)/i,
    /js(5|6|7)/i,
    /ecmascript 6/i,
    /ecript/i,
  ],
  JQUERY: [/jquery/i],
  KOTLIN: [/kotli(n|m)/i],
  LIGHTHOUSE: [/lighthouse/i],
  LINUX: [/linux/i],
  LOW_CODE: [/\blow( |-|\.)?code\b/i, /\bno( |-|\.)?code\b/i, /\bcronapp\b/i],
  LEAN: [/\blean\b/i],
  MACHINE_LEARNING: [/machine learning/i, /aprendizado de maquina/i],
  MAGENTO: [/magento/i],
  MARKETING: [
    /google analytics/i,
    /google tag manager/i,
    /inbound marketing/i,
    /e( |-)?mail marketing/i,
  ],
  MATERIAL_UI: [
    /\bmui\b/i,
    /\bmaterial?( |-|\.)?(ui|design)\b/i,
    /biblioteca(s)? de ui/i,
    /ui( |-|\.)?kit/i,
  ],
  MENSAGERIA: [
    /mensageria/i,
    /(spring )?kafka/i,
    /rab(b)?it(t)?mq/i,
    /message brokers/i,
    /amazon sqs/i,
    /pub(\/|-|\s)?sub/i,
    /activemq/i,
    /\bsqs\b/i,
    /arquitetura(s)?( distribuida e)? orientada(s)? a evento(s)?/i,
  ],
  MICRO_FRONTENDS: [/micro( |-|\.)?front( |-|\.)?end(s)?/i],
  MICRO_SERVICES: [/micro(s)?( |-|\.)?servic(o|e)(s)?/i],
  MOBILE_DEVELOPMENT: [
    /(desenvolvimento|conhecimento em) (de (aplicativos|aplicacoes) )?(mobile|nativo|move(is|l))/i,
    /(mobile|programacao) (development|nativ(o|a))/i,
    /\bios\b/i,
    /android/i,
    /mobile and web/i,
    /jetpack/i,
    /ml( |-|\.)?kit/i,
    /(aplica(tivo|coe|cao)(s)?|desenvolvimento) (move(is|l)|mobile(s)?)/i,
    /react( |-|\.)?native/i,
    /watermelondb/i,
  ],
  NEST: [/\b\bnest(( |-|\.)?js)?\b/i],
  NEXT: [/\bnext(( |-|\.)?js)?(?! (steps|generation|in))\b/i],
  NETWORK: [
    /\bcpe\b/i,
    /\bgpon\b/i,
    /wi( |-|\.)?fi/i,
    /dhcp/i,
    /\bnat\b/i,
    /noc(ao|oes) de rede(s)?/i,
    /network protocols/i,
  ],
  NUXT: [/nuxt(( |-|\.)?js)?/i],
  NODE: [/node(( |-|\.)?js)?(?!shift)/i],
  OBJECTIVE_C: [/\bobjective( |-|\.)?c\b/i],
  ORM: [/typeorm/i, /prisma/i],
  PACKAGE_MANAGER: [/npm/i, /yarn/i],
  PAYMENT_INTEGRATIONS: [/(integracao|gateway)(s)? de pagamento/i],
  PENTEST: [/burp( suite)?/i, /checkmarx/i, /pentest/i],
  PERFORMANCE_OPTIMIZATION: [
    /code( |-|\.)?splitting/i,
    /lazy( |-|\.)?loading/i,
    /optimize applications/i,
    /baixa latencia/i,
    /\bseo\b/i,
    /(alta disponibilidade|otimizacao) (d)?e (desempenho|performance)/i,
    /solucoes (consistentes )?em performance/i,
    /(seguranca e )?performance (para|na|de) (web|front( |-|\.)?end)/i,
    /performance e tempo de carregamento/i,
    /(web )?performance optimi(z|s)ation/i,
    /\bwpo\b/i,
    /hydration/i,
    /optimize (application )?performance/i,
    /render cycle/i,
    /event loop/i,
    /performance (profiling|web)/i,
  ],
  PERL: [/\bperl\b/],
  PHONEGAP: [/phonegap/i],
  PHP: [
    /\bphp\b/i,
    /\blaravel\b/i,
    /\bsymfony\b/i,
    /\bcodeigniter\b/i,
    /\bslim\b/i,
    /\btwig\b/i,
    /\bzend\b/i,
  ],
  PIXEL_PERFECT: [/pixel( |-|\.)?perfect/i],
  POSTMAN: [/postman/i],
  POWER_BI: [/power( |-|\.)?(bi|platform)/i, /looker/i, /microsoft fabric/i],
  PRISMA: [/prisma/i],
  PRODUCT_MANAGER: [/product (manager|owner)/i, /scrum master/i],
  PROGRAMMING_LOGIC: [/logica de programacao/i, /programming logic/i],
  PROTOTYPING: [
    /figma/i,
    /adobe( |-|\.)?xd/i,
    /photo( |-|\.)?shop/i,
    /prototipo(s)?/,
  ],
  PYTHON: [/python/i, /phyton/i, /\bpandas\b/i],
  PWA: [/\bpwa\b/i, /progressive web app(s)?/i],
  RAZOR: [/razor/i],
  REACT: [/react(( |-|\.)?js)?(?! native)(?! hooks)(?!tive)/i, /reacjs/i],
  REACT_HOOKS: [/react( |-|\.)?hook(s)?(?! form)/i],
  REACT_ROUTER: [/react router/i],
  REASONML: [/reasonml/i],
  RESPONSIVE_DESIGN: [
    /(design|desenvolvimento) (respons|adaptat)ivo/i,
    /responsive design/i,
    /(site(s)?|aplicac(oes|ao)|pagina(s)?|layout(s)?|interface(s)?( de usuario)?|projeto(s)?|) (interativas e |web )?(responsiv|adaptativ)(o|a)(s?)/i,
    /responsividade/i,
    /mobile( |-|\.)?(first|friendly)/i,
    /diferentes dimensoes de dispositivos/i,
    /altamente responsivas/i,
    /responsivas/i,
    /responsive/i,
    /portavel/i,
  ],
  RUBY: [/\bruby((-| )on(-| )rails)?\b/i, /\brails\b/i, /\brspec\b/i],
  RUST: [/\brust\b/i],
  SALESFORCE: [/salesforce/i],
  SAP: [/\bsap\b/i, /hybris/i, /\babap\b/i],
  SAS: [/\bsas\b/i],
  SASS: [/\bsass\b/i],
  SCALA: [/\bscala\b/i],
  SCRIPT: [/powershell/i, /bash( script)?/i, /automatizacao de processos/i],
  SITE_RENDERING: [
    /\bssr\b/i,
    /server( |-|\.)?side( |-|\.)?rendering/i,
    /\bssg\b/i,
    /static( |-|\.)?site( |-|\.)?generation/i,
    /\bisg\b/i,
    /incremental( |-|\.)?static( |-|\.)?regeneration/i,
    /\bcsr\b/i,
    /client( |-|\.)?side( |-|\.)?rendering/i,
  ],
  SPA: [/\bspa\b/, /single page applications/i],
  SPANISH: [/espanhol(?!a)/i, /spanish/i],
  STATE_MANAGEMENT: [
    /redux/i,
    /mobx/i,
    /ngrx/i,
    /rxjs/i,
    /state management/i,
    /use( |-|\.)?context/i,
    /(bibliotecas de )?(gerencia(mento|dor)|gestao) de estado(s)?/i,
    /recoil/i,
    /zustand/i,
    /\bjotai\b/i,
  ],
  STORYBOOK: [/storybook/i, /documentacao de componentes/i],
  STRAPI: [/strapi/i],
  STYLED_COMPONENTS: [/styled( |-|\.)?component(s)?/i],
  SWAGGER: [/swag(g)?e(r)?/i],
  SWIFT: [/\bswift\b/i],
  TAILWIND: [/tailwind( |-|\.)?(css)?/i],
  TANSTAK: [/\btanstack\b/i],
  TECH_LEAD: [
    /te(ch|am)(\s|-)lead/i,
    /(gerenciar|coordenar) (a )?equipe/i,
    /(gerenciamento|lideranca|gestao) de projetos/i,
    /lideranca de equipe(s)?/i,
    /engineering manager/i,
    /head de tecnologia/i,
  ],
  TELECOMMUNICATIONS: [/vonage/i, /voip/i, /chime/],
  TEST: [
    /jest/i,
    /selenium/i,
    /junit/i,
    /cypress/i,
    /\btdd\b/i,
    /\be2e\b/i,
    /enzyme/i,
    /mocha/i,
    /(code )?coverage/i,
    /teste(s)? func(t)?iona(is|l)/i,
    /teste(s)? (unitario|automatizado|integrado)(s)?/i,
    /(ferramenta(s)?|automa(tiza)?cao) de teste(s)?/i,
    /karma/i,
    /jasmine/i,
    /cucumber/i,
    /teste(s)? (d)?e (qualidade|unidade|integracao|software|performance|depuracao|interface(s)?|componente(s)?|aplicac(oes|ao))/i,
    /(react\s)?testing(-| )(library|practices|strategies)/i,
    /(unit|integration|end-to-end)( and integration)? test(ing)?/i,
    /testar (sistemas|apis e servicos)/i,
    /automated (unit( |-|\.)?)?test(s)?/i,
    /testing (scenario(s)?|coverage|tool(s)?)/i,
    /playwright/i,
    /teste(s)? a( |-|.|\/|\\)?b\b/i,
    /vitest/i,
  ],
  TOMCAT: [/tomcat(\sserver)?/i],
  TOTVS: [/totvs/i, /advpl/i, /protheus/i, /tlpp/i],
  TYPESCRIPT: [/type( |-|\.)?script/i, /\bts\b/i, /typeschipt/i, /typscript/i],
  UI: [/\bui\b/i, /uiux/i, /uxui/i, /user interface/i],
  UTILITY_LIBRARY: [/lodash/i],
  UX: [/\bux\b/i, /uiux/i, /uxui/i, /usabilidade/i, /user experience/i],
  VANILLA: [/((javascript|js) )?vanilla(( |-|\.)?(js|javascript)|\b)/i],
  VITE: [/\bvite\b/i],
  VUE: [/\bvue(( |-|\.)?js)?\b/i, /vuetify/i, /\bvuex\b/i],
  WEB_HOOKS: [/web( )?hook(s)?/i],
  WEBRTC: [/webrtc/i, /daily.co/],
  WINDOWS_FORMS: [/(windows|web)( |-|\.)?forms/i],
  WORDPRESS: [/wo(r)?d( |-|\.)?press/i, /elementor/i],
};

export const BENEFITS_REGEX = {
  ANUAL_BONUS: [
    /bonus (anual|per year)/i,
    /a(n)?nual bonus/i,
    /bonificacao anual/i,
  ],
  BIRTHDAY_DAYOFF: [
    /(day( |-|\.)?off|(dia de )?folga)(( no| de| n(a|o) (semana|mes) do seu) aniversario)?/i,
  ],
  COURSE_HELP: [
    /curso de aperfeiçoamento profissional/i,
    /learning and development support/i,
    /programa de capacitacao/i,
    /alura/i,
    /(acesso|bolsa(s)?) (a|de) (cursos|estudo(s)?)/i,
    /aquisicao de livros e cursos/i,
    /(desconto|parceria) (em|com) (cursos|faculdade(s)?)/i,
    /(auxilio|incentivo|investimento|subsidio|subvencao) ((para|a|em) )?(educacao|certificac(ao|oes)|estudo(s)?|curso(s)?)/i,
    /reembolso (educacao|por certificac(ao|oes))/i,
    /desconto em universidades e cursos/i,
    /bolsa de estudo/i,
    /acesso a plataformas de ensino/i,
    /(parcerias|incentivo) educaciona(is|l)/i,
    /fg academy/i,
    /universidade corporativa/i,
    /parceria com (instituicoes de ensino|universidades)/i,
    /programa(s)? de incentivo a aprendizado(s)?/,
    /cursos para seu desenvolvimento/i,
    /reembolso para certificacoes/i,
    /educational allowance/i,
  ],
  CHRISTMAS_BASKET: [/cesta de natal/i],
  FLEXIBLE_HOURS: [
    /(horario(s)?|carga horaria|jornada) (de trabalho )?flexive(is|l)/i,
    /flexible (working\s)?(hours|schedule(s)?)/i,
    /flexibilidade de (horario|jornada)/i,
    /nosso horario e flexivel/i,
    /flexibilidade/i,
  ],
  FOURTEENTH_SALARY: [/14. salario/i],
  GYMPASS: [
    /(gym|total)( |-|\.)?pass/i,
    /(auxilio )?(?!from )(academia|atividade fisica)/i,
    /wellhub/i,
  ],
  HEALTH_OR_DENTAL_PLAN: [
    /(plano(s)?|convenio|assistencia(s)?) ((medic(o|a)|de saude) e )?(odontologic(o|a)|medic(o|a)|odonto)/i,
    /dental/i,
    /(plano|auxilio|convenio|beneficio) (de )?(saude|odontologico)/i,
    /health (and dental )?(insurance|plan|care)/i,
    /(servico(s)? de )?telemedicina/i,
    /(assist\.|consulta) (medica|odontologica)/i,
    /medical insurance/i,
  ],
  HOME_OFFICE_VOUCHER: [
    /(auxilio|subsidio|ajuda de custo) (para\s)?(atuacao em\s)?(home( |-|\.)?office|trabalho remoto|montagem do seu escritorio)/i,
    /remote work allowance/i,
    /voucher (home( |-|\.)?office|para kit setup)/i,
    /equipamento/i,
    /hardware setup/i,
  ],
  LIFE_INSURANCE: [/seguro (de vida|saude)/i, /life insurance/i],
  MATERNITY_LEAVE: [/licenca maternidade/i, /(paid )?parental leave/i],
  MEAL_VOUCHER: [
    /(vale|ajuda de custo para|auxilio)( |-|\.)?(alimentacao|refeicao)/i,
    /(caju|flash|swile)/i,
    /\bv(\.)?a(\.)?\b/i,
    /\bv(\.)?r(\.)?\b/i,
    /(to use with|flex) food/i,
    /meal (or food )?allowance/i,
    /ifood (multi)?( |-|\.)?beneficios/i,
  ],
  NOTEBOOK: [/note( |-|\.)?book/i],
  PAID_VACATIONS: [
    /ferias ((anuais|e feriados)\s)?remunerad(a|o)(s)?/i,
    /(descanso|recesso) (anual|remunerado)/i,
    /paid (annual leave|vacation)/i,
    /ferias e feriados/i,
    /ferias de \d+ dias/i,
    /feriados nacionais(e regionais)?/i,
    /vacation & holidays/i,
    /(weeks|months) of vacation per year/i,
  ],
  PET_HELP: [/(licenca|convenio) pet/i],
  PHARMACY_AGREEMENT: [/convenio farmacia/i, /desconto em medicamentos/i],
  PLR: [
    /\bplr\b/i,
    /(lucros|participacao) (e|ou|nos|de) (resultados|lucros)/i,
    /\bppr\b/i,
  ],
  PRIVATE_PENSION: [/previdencia privada/i],
  PRODUCTIVITY_BONUS: [/bonus de produtividade/i],
  PSYCHOLOGICAL_HELP: [
    /(atendimento|auxilio|apoio|(cuidado|desconto) com|suporte social e|orientacao|bem estar) (psicologic(o|a)|bem estar|psicoterapia|(a\s)?saude (mental|emocional))/i,
    /mental health/i,
    /sessoes de terapia/i,
    /psicoterapia online/i,
    /saude mental/i,
    /zenklub/i,
    /equilibrium/i,
    /psicologia viva/i,
    /moodar/i,
    /vittude/i,
    /desconto em telepsicologia/i,
    /wellz/i,
  ],
  REFERRAL_BONUS: [
    /(bonus|premiacao) (por\s)?indicacao/i,
    /program of indication/i,
    /indicacao premiada/i,
    /(premio|programa de) indicac(ao|oes)/i,
    /indique um colega e ganhe bonus/i,
  ],
  STOCK_OPTIONS: [/stock option(s)?/i],
  THIRTEENTH_SALARY: [/13(.|º)? salario/i, /decimo terceiro/i],
  TRANSPORTATION_VOUCHER: [
    /(vale|auxilio)( |-|\.)?(transporte|combustivel|estacionamento|mobilidade)/i,
    /\bv(\.)?t(\.)?\b/i,
  ],
  USD_SALARY: [/salary in usd/i, /pagamento em moeda estrangeira/i],
};

export const HIRING_REGIMES_REGEX = {
  CLT: [/\bclt\b/i],
  PJ: [
    /\bpj\b/i,
    /pessoa juridica/i,
    /prest\. de servico/i,
    /cooperado/i,
    /horista/i,
  ],
};

export const TYPES_REGEX = {
  REMOTE: [
    /(home|anywhere)( |-|\.)?(office|work)/i,
    /remoto(?!: nao)/i,
    /trabalhar de casa/i,
    /remot(e|a)(?!a)/i,
    /100\% home/i,
    /teletrabalho/i,
    /trabalho digital/i,
    /100\% remot(o|a|e)/i,
    /residir (no|na|em)/i,
  ],
  HYBRID: [
    /(?<!(perfil|ambiente(s)?)\s)\bhibrid(o|a)\b/,
    /hybrid(?! app)/i,
  ],
  FACE_TO_FACE: [
    /presencial(mente)?/i,
    /remoto: nao/i,
    /(residir|more|morar) (em|na)/i,
    /\bon( |-|\.)?site\b/i,
    /ida(s)? presencia(is|l)/i,
    /disponibilidade para mudanca/i,
  ],
};

export const SENIORITY_REGEX = {
  JUNIOR: [
    /(?<! mentor )junior(?! es)/i,
    /estagiario/i,
    /estagio(?! (s|do))/i,
    /\bjn\b/i,
    /\bjr\b/i,
    /trainee/i,
    /internship/i,
  ],
  MID_LEVEL: [/pleno(?! conhecimento)/i, /\bmid\b/i, /\bpl\b/i],
  SENIOR: [/senior(?!(idade|ity))/i, /\bsr\b/, /\bespecialista\b/],
};
