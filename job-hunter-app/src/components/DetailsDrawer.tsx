...
const DetailsDrawer = ({
  ...
  return (
    <Drawer
      title={
        <Typography.Link href={selectedJob?.url} target="_blank" copyable>
          {selectedJob?.title}
        </Typography.Link>
      }
      placement="right"
      ...
      <List size="small">
        <List.Item key="company">
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <ListItemInner title="Empresa">
              {selectedJob?.company}
            </ListItemInner>
            <ListItemInner title="uuid">
              <Typography.Text copyable>{selectedJob?.uuid}</Typography.Text>
            </ListItemInner>
          </div>
        </List.Item>
        <List.Item key="platform_type_hiringRegime">
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <ListItemInner title="Plataforma">
        ...
        {selectedJob?.benefits?.length ? (
          <List.Item key="benefits">
            <ListItemInner
              title={`Benefícios (${selectedJob?.benefitsRating})`}
            >
        ...
        {selectedJob?.country || selectedJob?.state || selectedJob?.city ? (
          <List.Item key="address">
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {selectedJob?.country ? (
                <ListItemInner title="País">
                  {selectedJob?.country}
                </ListItemInner>
              ) : null}
              {selectedJob?.state ? (
                <ListItemInner title="Estado">
                  {selectedJob?.state}
                </ListItemInner>
              ) : null}
              {selectedJob?.city ? (
                <ListItemInner title="Cidade">
                  {selectedJob?.city}
                </ListItemInner>
              ) : null}
            </div>
